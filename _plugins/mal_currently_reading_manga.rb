...
module Jekyll
  # Generator plugin to fetch currently reading manga from MyAnimeList.
  # Writes to _data/mal_currently_reading_manga.json on each successful build.
  ...
  class MalCurrentlyReadingMangaGenerator < Generator
    safe true

    MAL_USERNAME     = 'JLO64'
    MAL_MANGA_FIELDS = 'id,title,main_picture,my_list_status{score,num_chapters_read,num_volumes_read,start_date},num_chapters,num_volumes'
    ...
    def generate(site)
      ...
      entries = fetch_currently_reading(auth_header)
      return unless entries

      images_dir = File.join(site.source, 'assets', 'images', 'mal')
      FileUtils.mkdir_p(images_dir)

      manga = entries.filter_map do |entry|
        node = entry['node']
        next unless node
      ...
      File.write(File.join(data_dir, 'mal_currently_reading_manga.json'), JSON.pretty_generate(manga))
      Jekyll.logger.info "MAL Manga:", "Synced #{manga.size} currently reading manga to mal_currently_reading_manga.json"
    end
