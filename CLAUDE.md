...
| `assets/images/hardcover/` | Book cover images (auto-generated, gitignored) |

## Custom Plugins (`_plugins/`)

- **`hardcover_currently_reading.rb`** — Fetches currently-reading books from Hardcover's GraphQL API at build time. Requires `HARDCOVER_TOKEN` (or `HARDCOVER_AUTHORIZATION`) env var. Downloads and converts cover images to WebP. Writes to `_data/hardcover_currently_reading.json`.
...
- Excluded from build: `docs/`, `_data/most_recent_mastodon_post.yml`
...
- Hardcover book grid styles are in `_sass/hardcover.scss`
